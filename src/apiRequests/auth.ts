...
const authAPI = {
  ...
  setCookiesReq: null as Promise<{
    status: number
    payload: RefreshTokenResType["data"]
  }> | null,
  loginServer: (body: LoginBodyType) =>
    http.post<LoginResType>("users/login", body),
  loginClient: (body: LoginBodyType) =>
    http.post<LoginResType>("api/auth/login", body, {
      baseUrl: ""
    }),
  logoutServer: (body: { refreshToken: string; accessToken: string }) =>
  ...
  refreshTokenServer: (body: RefreshTokenBodyType) =>
    http.post<RefreshTokenResType>("auth/refresh-token", body),
  async refreshTokenClient() {
    if (this.refreshTokenReq) {
      return this.refreshTokenReq
    }
    this.refreshTokenReq = http.post<RefreshTokenResType>(
  ...
  async setCookies(body: RefreshTokenResType["data"]) {
    ...
    this.setCookiesReq = http.post<RefreshTokenResType["data"]>(
